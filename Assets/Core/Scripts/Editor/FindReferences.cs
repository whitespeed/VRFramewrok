    ...
    static private void Find()
    {
        ...
        if (!string.IsNullOrEmpty(path))
        {
            ...
            EditorApplication.update = delegate ()
            {
                ...
                bool isCancel = EditorUtility.DisplayCancelableProgressBar("Finding References", file, (float)startIndex / (float)files.Length);

                if (Regex.IsMatch(File.ReadAllText(file), guid))
                {
                    Debug.Log(string.Format("{0} referenced by {1}", Selection.activeObject,file), AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(GetRelativeAssetsPath(file)));
                }
...
public partial class FindReferencesByStaticClass
{

    [MenuItem("Assets/Show References In StaticClass", false, 28)]
    static void StaticRef()
    {
        ...
        {
            Debug.LogError("Please use it in playing mode.");
        }
                ...
                foreach (Type type in assembly.GetTypes())
                {
                    try
                    {
                        ...
                        if (assetPaths.Count > 0)
                        {
                            ...
                            sb.AppendFormat("Assets referenced by {0}.cs\n", type.ToString());
